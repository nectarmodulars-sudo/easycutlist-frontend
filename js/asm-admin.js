...
router.get('/users-detailed', async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const [{ data: authData, error: authErr }, { data: subs, error: subErr }, { data: profs }] = await Promise.all([
      ...
      _sb.from('profiles').select('id,plan,plan_expires_at,tier_id')
    ]);
    ...
    const users = authData.users.map(u => {
      ...
      // Optimizer plan from profiles
      const pr = profMap[u.id];
      let optPlan = 'free', optExpiresAt = null;
      if (pr) {
        optExpiresAt = pr.plan_expires_at;
        const optActive = pr.plan === 'pro' && pr.plan_expires_at && new Date(pr.plan_expires_at) > now;
        optPlan = optActive ? 'pro' : (pr.plan === 'pro' ? 'expired' : 'free');
      }
      return {
        id: u.id,
        ...
        optPlan, optExpiresAt,
        tierId: pr ? (pr.tier_id || null) : null
      };
    });
...
// Grant custom catalogue access (optional per-user display_name alias)
...
function autoParseItem(wb, ws, rangeInfo) {  const rangeRef = XLSX.utils.decode_range(rangeInfo.range);
  ...
  for (let r = startRow; r <= endRow && !isManual; r++) {
    // Skip the note row — prose there may legitimately contain the word "manual"
    const labelC = ws[XLSX.utils.encode_cell({ r, c: 0 })];
    if (labelC && String(labelC.v || '').trim().toLowerCase() === 'note') continue;
    for (let c = 0; c <= 3; c++) {
      const cell = ws[XLSX.utils.encode_cell({ r, c })];
      const txt = cell ? String(cell.v || '').trim().toLowerCase() : '';
      // Only a short marker cell counts, not a sentence that mentions the word.
      if (txt.length <= 20 && /\bmanual\b/.test(txt)) { isManual = true; break; }
    }
  }
...
// ══ OPTIMIZER Plan management (table: pricing) ══
// Separate from ASM. Supports feature-matrix plans: each plan carries a
// features {} object of flag_key -> true. Local admin only.
router.get('/opt-pricing', async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { data, error } = await _sb.from('pricing').select('*').order('months', { ascending: true });
    if (error) throw error;
    res.json({ success: true, plans: data || [] });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/opt-pricing/save', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { plan_id, months, price_inr, label, active, features, sort_order } = req.body || {};
    if (!plan_id || !/^[a-z0-9_]+$/.test(plan_id)) {
      return res.status(400).json({ error: 'plan_id required (lowercase letters/numbers/underscore)' });
    }
    if (!(Number(months) > 0))     return res.status(400).json({ error: 'months must be a positive number' });
    if (!(Number(price_inr) >= 0)) return res.status(400).json({ error: 'price_inr must be >= 0' });
    const feat = (features && typeof features === 'object') ? features : {};
    const { error } = await _sb.from('pricing').upsert({
      plan_id,
      ...
      active:     active !== false,
      features:   feat,
      sort_order: Number(sort_order) || Number(months),
...
router.post('/opt-pricing/delete', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { plan_id } = req.body || {};
    if (!plan_id) return res.status(400).json({ error: 'plan_id required' });
    const { error } = await _sb.from('pricing').delete().eq('plan_id', plan_id);
    if (error) throw error;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ══ TIER SYSTEM (tables: tiers, tier_pricing) ══
// Tier = feature-set + display. Pricing = per tier per duration. Local admin only.
router.get('/tiers', async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { data: tiers, error: e1 } = await _sb.from('tiers').select('*').order('rank', { ascending: false });
    if (e1) throw e1;
    const { data: pricing, error: e2 } = await _sb.from('tier_pricing').select('*').order('months', { ascending: true });
    if (e2) throw e2;
    res.json({ success: true, tiers: tiers || [], pricing: pricing || [] });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/tiers/save', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { tier_id, name, subtitle, badge, rank, features, active } = req.body || {};
    if (!tier_id || !/^[a-z0-9_]+$/.test(tier_id)) {
      return res.status(400).json({ error: 'tier_id required (lowercase letters/numbers/underscore)' });
    }
    if (!name) return res.status(400).json({ error: 'name required' });
    const feat = (features && typeof features === 'object') ? features : {};
    const { error } = await _sb.from('tiers').upsert({
      tier_id,
      name,
      subtitle: subtitle || '',
      badge:    badge || '',
...
router.post('/tiers/delete', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { tier_id } = req.body || {};
    if (!tier_id) return res.status(400).json({ error: 'tier_id required' });
    // tier_pricing rows cascade-delete via FK
    const { error } = await _sb.from('tiers').delete().eq('tier_id', tier_id);
    if (error) throw error;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Save one tier+duration price. Body: { tier_id, months, price_inr, active }
router.post('/tiers/price', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { tier_id, months, price_inr, active } = req.body || {};
    if (!tier_id) return res.status(400).json({ error: 'tier_id required' });
    if (!(Number(months) > 0))     return res.status(400).json({ error: 'months must be positive' });
    if (!(Number(price_inr) >= 0)) return res.status(400).json({ error: 'price_inr must be >= 0' });
    const { error } = await _sb.from('tier_pricing').upsert({
      tier_id,
      months:    Number(months),
      price_inr: Number(price_inr),
      active:    active !== false,
      updated_at: new Date().toISOString(),
    }, { onConflict: 'tier_id,months' });
    if (error) throw error;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/tiers/price-delete', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { tier_id, months } = req.body || {};
    if (!tier_id || !months) return res.status(400).json({ error: 'tier_id and months required' });
    const { error } = await _sb.from('tier_pricing').delete().eq('tier_id', tier_id).eq('months', Number(months));
    if (error) throw error;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/user-set-plan', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    let { user_id, plan, months, days, planId, extend } = req.body || {};
    ...
    if (!(m > 0) && !(d > 0)) return res.status(400).json({ error: 'months, days, or a valid planId required' });

    // extend = add duration to existing expiry if still active; else from now
    let base = new Date();
    if (extend) {
...
router.get('/coupons', async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { data, error } = await _sb.from('asm_coupons').select('*').order('created_at', { ascending: false });
    if (error) throw error;
...
router.post('/coupons/save', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    let { code, type, value, scope, allowed_emails, plan_ids, expires_at, max_uses, active } = req.body || {};
    code = (code || '').trim().toUpperCase();
    if (!code) return res.status(400).json({ error: 'Coupon code required' });
    if (type !== 'percent' && type !== 'flat') return res.status(400).json({ error: "type must be 'percent' or 'flat'" });
    if (!(Number(value) > 0)) return res.status(400).json({ error: 'value must be positive' });
    if (type === 'percent' && Number(value) > 100) return res.status(400).json({ error: 'percent cannot exceed 100' });
    scope = scope === 'restricted' ? 'restricted' : 'public';

    const emails = scope === 'restricted'
      ? (Array.isArray(allowed_emails) ? allowed_emails : String(allowed_emails || '').split(',').map(s => s.trim().toLowerCase()).filter(Boolean))
      : null;
    const plans = (plan_ids && (Array.isArray(plan_ids) ? plan_ids.length : String(plan_ids).trim()))
      ? (Array.isArray(plan_ids) ? plan_ids : String(plan_ids).split(',').map(s => s.trim()).filter(Boolean))
      : null;

    const row = {
      code, type, value: Number(value), scope,
    ...
    const { error } = await _sb.from('asm_coupons').upsert(row, { onConflict: 'code' });
    if (error) throw error;
    res.json({ success: true, message: 'Coupon saved', code });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Toggle active
router.post('/coupons/toggle', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { code, active } = req.body || {};
    if (!code) return res.status(400).json({ error: 'code required' });
    const { error } = await _sb.from('asm_coupons').update({ active: !!active }).eq('code', String(code).toUpperCase());
    if (error) throw error;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// Delete
router.post('/coupons/delete', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const { code } = req.body || {};
    if (!code) return res.status(400).json({ error: 'code required' });
    const { error } = await _sb.from('asm_coupons').delete().eq('code', String(code).toUpperCase());
    if (error) throw error;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ══ OPTIMIZER (core EasyCutList) plan management — writes to `profiles` ══
// POST /opt-set-plan  { user_id, plan?, tier_id?, months?, days?, extend? }
router.post('/opt-set-plan', express.json({ limit: '1mb' }), async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    let { user_id, plan, tier_id, months, days, extend } = req.body || {};
    if (!user_id) return res.status(400).json({ error: 'user_id required' });
    plan = plan || 'pro';

    if (plan === 'free') {
      const { error } = await _sb.from('profiles').update({ plan: 'free', plan_expires_at: null, tier_id: null }).eq('id', user_id);
      if (error) throw error;
      return res.json({ success: true, plan: 'free' });
    }
    ...
    const m = Number(months) || 0;
    const d = Number(days) || 0;
    if (!(m > 0) && !(d > 0)) return res.status(400).json({ error: 'months or days required' });

    let base = new Date();
    if (extend) {
      const { data: cur } = await _sb.from('profiles').select('plan_expires_at').eq('id', user_id).single();
      if (cur && cur.plan_expires_at && new Date(cur.plan_expires_at) > base) base = new Date(cur.plan_expires_at);
    }
    const expiresAt = new Date(base);
    if (m > 0) expiresAt.setMonth(expiresAt.getMonth() + m);
    if (d > 0) expiresAt.setDate(expiresAt.getDate() + d);

    const upd = { plan: 'pro', plan_expires_at: expiresAt.toISOString() };
    if (tier_id) upd.tier_id = tier_id; // assign specific tier's feature-set
    const { error } = await _sb.from('profiles')
      .update(upd)
      .eq('id', user_id);
    if (error) throw error;
    res.json({ success: true, plan: 'pro', tier_id: tier_id || null, expiresAt: expiresAt.toISOString() });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ============================================================================
// GET /asm/admin/analytics-data — per-user ASM button usage + projects saved
// ============================================================================
router.get('/analytics-data', async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ error: 'Supabase not configured' });
    const [{ data: authData, error: authErr }, ana, proj] = await Promise.all([
      _sb.auth.admin.listUsers({ perPage: 1000 }),
      _sb.from('asm_analytics')
         .select('user_id,export_optimizer_count,make_quotation_count,export_pdf_count,updated_at'),
      _sb.from('asm_projects').select('user_id')
    ]);
    if (authErr) throw authErr;

    const anaMap = {};
    (ana.data || []).forEach(r => { anaMap[r.user_id] = r; });
    const projMap = {};
    (proj.data || []).forEach(p => { projMap[p.user_id] = (projMap[p.user_id] || 0) + 1; });

    const rows = authData.users.map(u => {
      const a = anaMap[u.id] || {};
      return {
        email: u.email || '(no email)',
        exportOptimizer: a.export_optimizer_count || 0,
        makeQuotation:   a.make_quotation_count   || 0,
        exportPdf:       a.export_pdf_count        || 0,
        projectsSaved:   projMap[u.id]             || 0,
        lastUsed:        a.updated_at              || null
      };
    });
    rows.sort((x, y) =>
      (y.exportOptimizer + y.makeQuotation + y.exportPdf + y.projectsSaved) -
      (x.exportOptimizer + x.makeQuotation + x.exportPdf + x.projectsSaved)
    );
...
// ============================================================================
// SUPPORT — Item problem messages (Phase 1, text threads)
// ============================================================================
router.get('/problems', async (req, res) => {
  try {
    if (!_sb) return res.status(500).json({ success:false, error:'Supabase not configured' });
    const { data: problems, error } = await _sb.from('asm_problems')
      .select('*').order('updated_at', { ascending: false });
    if (error) throw error;
    const ids = (problems||[]).map(p=>p.id);
    let msgs = [];
    if (ids.length) {
      const { data: m, error: me } = await _sb.from('asm_problem_messages')
        .select('*').in('problem_id', ids).order('created_at', { ascending: true });
      if (me) throw me;
      msgs = m || [];
    }
    const byPid = {};
    msgs.forEach(m => { (byPid[m.problem_id] = byPid[m.problem_id] || []).push(m); });
    const rows = (problems||[]).map(p => ({ ...p, messages: byPid[p.id] || [] }));
...
router.post('/problems/reply', express.json({ limit:'12mb' }), async (req, res) => {
  try {
    ...
    await _sb.from('asm_problems')
      .update({ unread_admin: false, unread_user: true, updated_at: new Date().toISOString() })
      .eq('id', problem_id);

    // notify the user via existing bell system
    const { data: prob } = await _sb.from('asm_problems')
      .select('user_id, item_name').eq('id', problem_id).single();
    if (prob && prob.user_id) {
      await _sb.from('asm_notifications').insert({
        recipient_id: prob.user_id,
        type: 'problem_reply',
        title: 'Reply to your reported problem',
        body: prob.item_name ? ('Re: ' + prob.item_name) : 'You have a reply from support.'
      });
    }
    res.json({ success:true });
  } catch (e) { res.status(500).json({ success:false, error: e.message }); }
...
router.post('/problems/mark-read', express.json({ limit:'256kb' }), async (req, res) => {
  try {
    if (!_sb) throw new Error('Supabase not configured');
    const problem_id = Number((req.body||{}).problem_id);
    if (!problem_id) throw new Error('problem_id required');
    await _sb.from('asm_problems').update({ unread_admin: false }).eq('id', problem_id);
    res.json({ success:true });
  } catch (e) { res.status(500).json({ success:false, error: e.message }); }
});

router.post('/problems/close', express.json({ limit:'256kb' }), async (req, res) => {
  try {
    if (!_sb) throw new Error('Supabase not configured');
    const problem_id = Number((req.body||{}).problem_id);
    if (!problem_id) throw new Error('problem_id required');
