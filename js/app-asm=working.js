...
const ASMModule = (() => {
  ...
  async function recalc(instanceId) {
    ...
    try {
      ...
      if (data.success) {
        // Preserve manually-added rows (+Add Rows) — recompute only regenerates
        // formula-driven outputs, so re-append the user's manual rows after.
        const manualRows = (inst.outputs || []).filter(o => o._manual);
        inst.outputs = data.outputs.concat(manualRows);
